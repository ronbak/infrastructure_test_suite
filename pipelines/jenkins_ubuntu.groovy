node {
  ...
  stage ('DeployJenkins-Core'){
    ...
    string(credentialsId: 'nonprd-storage-account-key', variable: 'AZURE_STORAGE_ACCOUNT_KEY'),]) {
      ...
      sh "ruby infrastructure_test_suite/bin/provision.rb --action deploy --environment core --config arm_templates/vms/jenkins_ubuntu.config.json --complete --prep_templates"      
    }
  }
}